...
def translate_three_coloring(graph):
    ...
    for (x_i, x_j) in edges:
        clauses = []
        clauses.append([3*x_i +1, 3*x_i +2, 3*x_i + 3])
        clauses.append([-(3*x_i + 1), -(3*x_i + 2)])
        clauses.append([-(3*x_i + 2), -(3*x_i + 3)])
        clauses.append([-(3*x_i + 1), -(3*x_i + 3)])
        
        clauses.append([3*x_j + 1, 3*x_j + 2, 3*x_j + 3])
        clauses.append([-(3*x_j + 1), -(3*x_j + 2)])
        clauses.append([-(3*x_j + 2), -(3*x_j + 3)])
        clauses.append([-(3*x_j + 1), -(3*x_j + 3)])
        
        clauses.append([-(3*x_i + 1), -(3*x_j + 1)])
        clauses.append([-(3*x_i + 2), -(3*x_j + 2)])
        clauses.append([-(3*x_i + 3), -(3*x_j + 3)])
        
        for i in clauses:
            if i not in s.clauses:
                s.add_clause(i)
    return s
...
def extract_graph_coloring_from_truth_assignment(graph, truth_assign):
    ...
    for i in range(graph.n):
        if(truth_assign[3*i +1] == True):
            ret[i] = 1
        elif(truth_assign[3*i +2] == True):
            ret[i] = 2
        else:
            ret[i] = 3
    return ret
